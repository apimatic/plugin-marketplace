...
A plugin whose skills teach a coding agent to install and use the APIMatic-generated **PayPal Server SDK SDK**, in C#/.NET. Every SDK fact the skills state is grounded in the SDK's own source and generated documentation, not in what a model remembers about this API.
...
/plugin install sdk-skills-test-dev
```
...
codex plugin add sdk-skills-test-dev@<marketplace>
```

`<marketplace>` is the name the directory you just added registers under — `codex plugin marketplace list` prints it.

Once your team publishes it to a plugin marketplace, add that marketplace in place of the local path and install `sdk-skills-test-dev` from it by name.
