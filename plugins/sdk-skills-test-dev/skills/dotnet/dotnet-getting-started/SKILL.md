...
description: "PayPal Server SDK .NET SDK identity and lookup layer (.NET/C# only) you load directly — install, root namespace, environments, auth pattern, and the SDK map (shipped inside the SDK source repo): every operation signature and error case by lookup, plus the source file declaring each model, enum and error type. Load it for every contract fact, never memory; dotnet-integrate-pay-pal-server-sdk is the workflow that uses it."
---

# Getting started with the PayPal Server SDK .NET SDK

> **Who this skill is for.** This is the **map layer**: load it to ground every PayPal Server SDK SDK fact. `dotnet-integrate-pay-pal-server-sdk` is the workflow that drives it — you collect the contracts for the work in scope into `pay-pal-server-sdk-plan.md`, implement from that sheet, and come back to the one map page a row cites for any fact the sheet is missing. Every contract fact comes from here, never from memory.
...
| Source repo | https://github.com/Alishahzad1903/sdk-skills-test-csharp-v4-dev (branch `main` — the ref this map documents) |
...
| Log environment variable | `PAYPALSERVERSDKCLIENT_LOG` |
| Target framework | `netstandard2.0` |

The table above is **orientation, not a copy-paste recipe** — it gives you the names and facts (install, namespaces, the auth *pattern*, the environments), while the actual integration code comes from the companion skills. Load every one the contract sheet's REQUIRED READING names before you start implementing (`dotnet-integrate-pay-pal-server-sdk` makes that binding); the **Integration workflow** below says which governs which step.

## Namespaces (using-directives)

The SDK splits its public types across **separate child namespaces**. C# does **not** import child namespaces transitively, so `using PayPalServerSdk.Models;` alone does **not** make enums, union types, or error types visible — you get `CS0103`/`CS0246` ("name/type does not exist") on build. Add a separate `using` for each kind of type you reference:

| Type | Namespace |
| --- | --- |
| `PayPalServerSdkClient` | `PayPalServerSdk` |
| `PayPalServerSdkClientOptions` | `PayPalServerSdk` |
...
- **`map/operations/`** — one page per controller, one block per operation: the exact C# signature with its request record's required members, the return type, the error case (A: typed `{Operation}Error` with its `TryGet…` accessors and their statuses / B: `RawError`), pagination and server-group bullets **only where they differ from the defaults**, and a **Type sources** table naming the file that declares every type the operation mentions.
- **Shapes live in the source, one type per file.** Every file under `Models/`, `Errors/` and `Requests/` declares exactly one public type named after the file, so a type name *is* its path: take it from the operation's Type sources table (or the kind's directory — records `Models/`, enums `Models/Enums/`, typed errors `Errors/`, request records `Requests/`, or `Requests/<Controller>/` for an operation on a controller; an operation with several tags is filed under the first tag it declares) and read the fields, `required` flags, enum values and, on a model, the `[JsonPropertyName]` wire names from that one file (a request record is never serialized and carries none). The compiler is the backstop: a wrong name fails to build.
...
2. **Silence means the default.** The index's defaults table states what holds for every operation (throw-only, no pagination, the default server group, inputs as one request record); an operation's block states only what differs. A block silent on pagination has none — take the default and move on rather than opening the source to confirm it.
...
4. The **HTTP verb/route and the endpoint's behavioural prose** live on the operation method itself — the XML `<remarks>` docs in the `Api/{Controller}.cs` file the page header names; what each input means is in the per-member docs on its request record. Read them there when wire-level detail or call-acceptance semantics matter; never reconstruct a route from memory.

Keep lookups cheap — the rules that keep a session's context small:

- Collect the contracts for **every** in-scope operation in **one** pass — signatures and error accessors from the map pages, then required fields with wire names and enum values from the declaring files those pages name — into the **contract sheet** in `pay-pal-server-sdk-plan.md`, then implement from the sheet. Do not re-open a map page or source file per field, and never re-look-up a fact the sheet already carries.
...
The SDK source is where the map lives, so **getting it is the first step of SDK work this session, not a fallback**: one shallow clone at the recorded ref, into the **system temp directory** (`<temp>/pay-pal-server-sdk-sdk-src/`), reused for the whole session. The clone is a read-only reference, never in the project repo, and **not** a build dependency. Opening the **one declaring file** a map page names is the normal flow here — that is where model, enum and error shapes live. What stays off-limits is *locating* anything by scanning: every file you open is one the map named, opened scoped, never found by grep.

**Clone once, up front**, into a fresh timestamped folder under `<temp>/pay-pal-server-sdk-sdk-src/`, and reuse that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/pay-pal-server-sdk-sdk-src/$(date +%Y%m%d-%H%M%S)"
git clone --filter=blob:none --branch main https://github.com/Alishahzad1903/sdk-skills-test-csharp-v4-dev "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\pay-pal-server-sdk-sdk-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --filter=blob:none --branch main https://github.com/Alishahzad1903/sdk-skills-test-csharp-v4-dev $dir
# Reuse $dir for the rest of your session (it is your clone path).
```
...
The clone lives in `<temp>/pay-pal-server-sdk-sdk-src/`, **never** in the project repo, and its path never goes into `pay-pal-server-sdk-plan.md` — the plan must stay portable, and the clone is a session-local reference, not part of the project.
...
- `Api/` — one file per controller/group; **this is where the operation methods live** (the HTTP verb and route, and the XML `<remarks>` docs carrying the endpoint's behavioural prose). The map's per-controller pages name the exact file.
- `Models/` (+ `Models/Enums/`) — body and response models, unions and enums; one public type per file, named after the file.
- `Requests/` — one request record per operation that takes input, carrying the per-member docs for each input; a controller's records sit in `Requests/<Controller>/` (an operation with several tags is filed under the first tag it declares).
...
- `Core/` — HTTP infrastructure (the `SdkException` family, `RawError`, auth, retries, `SdkHook`).
