---
name: integrate-visa
description: MANDATORY FIRST STEP for Visa/CyberSource .NET SDK work in a C#/.NET project — load this BEFORE spawning the visa-sdk agent, not after; .NET/C# SDK ONLY, never load it for any other language. Applies when asked to integrate Visa or CyberSource in C# — raise, deliver, correct or cancel invoices, take payments, capture or refund them, manage stored payment tokens, search transactions, run reports — or when a Visa/CyberSource .NET SDK call errors or behaves unexpectedly. Knowing to delegate to the visa-sdk agent is NOT a substitute for loading this, because it carries four binding gates that are stated NOWHERE else and that you cannot infer from the agent's own description — (1) the exact plan-file path you must dictate to the agent, (2) the no-project-file-edits window while the agent runs, (3) the hard gate that the plan file exists and has been read before any code, and (4) the mandatory load of every dotnet-* companion skill the returned contract sheet names. Skipping it means project files get edited while the agent is still writing them, and the companion skills its contract sheet names never get loaded.
---

# Visa/CyberSource .NET SDK — Router (map + one agent)

You (the main agent) orchestrate; the `visa-sdk` agent carries the SDK knowledge. The
...
yourself. The `dotnet-*` companion skills are a different thing: they are *usage* guidance,
they are yours to load, and Step 1c below makes loading them mandatory.

## The subagent

- **`visa-sdk`** is the single agent for every SDK need. It grounds in a bundled SDK map
  ...
  its whole map context from scratch (the dominant helper cost), and this SDK's map is large —
  1,972 record models across eleven pages and 135 operations across 49 controllers — so reuse
  is not optional.

**Scope guard:** the APIMatic-generated Visa/CyberSource **.NET SDK** (root namespace
`CyberSourceMergedSpec`) in **C#/.NET projects only**. Unrelated API, or any language other
than C#/.NET — do nothing; this router and its agent do not apply.

## Workflow

**If the user opens with a reported SDK error or unexpected Visa/CyberSource behaviour** (not
new feature work), spawn **`visa-sdk`** directly with the error output and the files involved,
...
Your FIRST action is to spawn **`visa-sdk`** once, with the user's full request (all
features in scope — one spawn covers the whole implementation). Dictate the output path in
the brief: the absolute path where it writes the plan (`<project repo root>/visa-plan.md`) —
do not let it pick its own location.

It writes `visa-plan.md` (plan + contract sheet) and returns its path.

**Parallelize the wait — prerequisites only.** Repo reconnaissance (where the integration
lands in this codebase) is not the `visa-sdk` agent's job: kick it off in the SAME message
...
- locating the SDK reference and its pinned version in the project;
...
**`dotnet-authentication` is required reading for this SDK whether or not the sheet names it.**
Unlike every other APIMatic .NET SDK, this one declares no security scheme and its options
class carries no credentials property, so there is nothing to "set" and no 401 to debug by
inspecting an options object. Authentication is an opt-in hook configured entirely from
environment variables, and an integration that does not know that sends every request
unsigned while appearing to work locally.

These are usage skills; loading them is not the same as reading the map, and it does not
breach the map boundary. Contract *facts* still come only from the sheet or the warm agent.
...
- Blocker or major assumption → surface it to the user in plain language, get their answer,
  send the clarification to the EXISTING `visa-sdk` agent (re-spawn only if it is gone). It
...
ask the warm `visa-sdk` agent, never guess.

### Step 2 — Implement from the contract sheet

1. Read `visa-plan.md` once. Treat its contracts as authoritative — do not re-derive or
...
2. Implement sequentially, following the repo's own conventions and layering. You loaded the
   ...
   contract sheet or the warm `visa-sdk` agent — never re-derive one from a companion.
...
4. **Any compile or runtime error involving an SDK type or member** (`CS1061`, `CS0117`,
   `CS0234`, `CS0104`, `CS1503`, `CS7036`, … on `CyberSourceMergedSpec.*`, or a provider error
   at runtime) → send the exact error output and the files involved to your EXISTING
   `visa-sdk` agent (a follow-up message, NOT a new spawn), and wait. It fixes the code in
...
A standalone Visa/CyberSource question with no code change: ask the warm `visa-sdk` agent
(narrow-question mode) and relay its grounded answer. Never answer from memory, even for
"easy" questions.

## Wait for your agent

**Never create or edit a project file while `visa-sdk` is running** — it edits files in
...
- **Get SDK knowledge from the `visa-sdk` agent, not yourself.** Don't clone or decompile the
  SDK, don't fetch its source files, and don't web-search Visa or CyberSource topics to find an
...
- **Don't load `visa-getting-started` or the SDK map pages** — the map is the agent's, and
  loading it just bloats your context. This SDK's map is unusually large, so the cost of
  breaking this rule is unusually high. (The `dotnet-*` companions are the opposite case: load
  them, per Step 1c.) Don't re-derive a contract *fact* from a companion: exact signatures, wire
  names, error accessors, and enum values come from the contract sheet (or you ask the warm
  `visa-sdk` agent). Where a companion points at the SDK source or a clone, that path is the
  agent's, not yours — you have no local clone.
- **Never write a Visa/CyberSource or SDK fact from memory** — every signature, field name, enum
  value, and error type in your code must come from the contract sheet or a lookup. And **never
...
- **Don't re-derive or double-check a sheet row from memory.** If you are unsure what a row
  said, batch your questions into ONE message to the warm `visa-sdk` agent — it holds the map
  warm and answers in seconds.
- **Never spawn a second `visa-sdk` agent.** One spawn per session; everything after is a
