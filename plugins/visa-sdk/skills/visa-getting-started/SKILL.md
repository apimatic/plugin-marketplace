...
| | |
| --- | --- |
| API | Visa (CyberSource) |
| NuGet package | `APIMatic.VisaCyberSource` (install version-less — see *Install* below) |
| Source repo | https://github.com/asadali214/visa-cybersource-sample-sdk (tag `v2.0.1` — the release this map documents) |
| Root namespace | `CyberSourceMergedSpec` (the `using` namespace) |
| Client class | `CyberSourceMergedSpecClient` |
| Options class | `CyberSourceMergedSpecClientOptions` |
<!-- gen:auth -->
| Auth | No credentials property on the client options — see the SDK map's *Servers & auth* section |
<!-- /gen:auth -->
<!-- gen:environments -->
...
namespaces transitively, so `using CyberSourceMergedSpec.Models;` alone does **not** make enums, union
...
This SDK is published to NuGet; install it into the project that will call Visa:
...
- **`map/operations/{Controller}.md`** — one page per controller. Each operation
  ...
  Each page's header names the source file it came from (e.g. `Api/Invoices.cs`).
...
3. Only when you need a **full method or model body** the map doesn't carry, take the file path the map
   names (e.g. `Api/Invoices.cs`, `Errors/CreateInvoiceError.cs`, `Models/CreateInvoiceRequest.cs`) and
...
temp directory** (`<temp>/visa-sdk-src/`), never in the project
...
**Clone only on a real map-side issue.** Clone once this session — shallow, at tag `v2.0.1` (the exact
release the map was generated from; see the map's source-commit stamp) — into a fresh timestamped folder
under `<temp>/visa-sdk-src/`, and reuse that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/visa-sdk-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch v2.0.1 https://github.com/asadali214/visa-cybersource-sample-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\visa-sdk-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch v2.0.1 https://github.com/asadali214/visa-cybersource-sample-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/visa-sdk-src/`, **never** in the project repo, and its path never goes into
`visa-plan.md` — the main agent must never see the clone or its path.
...
- **Don't fetch GitHub files one at a time** as your way in — `…/blob/…` pages return HTML (not source) and
  ...
  never a `…/blob/…` page), e.g. `…/main/Api/Invoices.cs`.
...
- `Api/` — one file per controller/group; **this is where the operation methods and their signatures live**
  ...
  per-controller pages name the exact file (e.g. `Api/Invoices.cs`).
...
- `Servers/`, `CyberSourceMergedSpecClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new CyberSourceMergedSpecClient(...)`, build its options, or DI-register via
   `AddCyberSourceMergedSpecClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before the first call to any endpoint. This SDK
   declares no security scheme, so `CyberSourceMergedSpecClientOptions` carries **no credentials property**
   and there is nothing to set on it. Every request is signed instead by an opt-in **HTTP Signature**
   `SdkHook` configured from four environment variables. (*The signature won't tell you:* the variables are
   read once, inside the client constructor, so they must be set **before** the client is constructed;
   leaving the switch unset does not disable auth, it sends every request unsigned.)
