...
description: "Videogen Python SDK identity and lookup layer (Python only) — install, import root, base URL/environments, the auth pattern, the SDK map that ships at the SDK root (`sdk-map.md` + `map/operations/`) and how to traverse it, and the module table naming the one file owning each fact the map leaves to the source. Load this before answering any Videogen Python SDK contract question or writing any SDK code."
---

# Getting started with the Videogen Python SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Videogen Python SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source modules it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `python-integrate-videogen` for the workflow that wraps this skill.
...
Verified against `videogen/` and `pyproject.toml` of the generated package at version `1.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.
...
| Version | `1.0.0` |
| Sync client class | `VideogenClient` (alias `Client`) |
| Async client class | `AsyncVideogenClient` (alias `AsyncClient`) |
...
| Environments | **no environment enum** — one `base_url` string, defaulting to `https://api.videogen.io` |
...
`videogen/__init__.py` exports exactly 5 names beside the `videogen.models` subpackage it re-exports:
...
This SDK has **no environment type and no environment constants**. There is one knob: `base_url`, on `ServerConfig` (`videogen/server/server_config.py`), and its default is **`https://api.videogen.io`**:

```python
base_url: str = "https://api.videogen.io"
```

Consequences to state on every contract sheet that touches configuration:

- Omitting `base_url` gives you **`https://api.videogen.io`**, silently. A caller who believes they configured a different host and did not gets that host's behaviour with live credentials.
...
| --- | --- | --- | --- |
| `client.account` | `Account` / `AsyncAccount` | 1 | `get_me` |
| `client.assistant` | `Assistant` / `AsyncAssistant` | 5 | `act_on_assistant_action` · `get_assistant` · `get_assistant_message` · `send_assistant_message` · `start_assistant_chat` |
| `client.entities` | `Entities` / `AsyncEntities` | 7 | `add_entity_reference` · `archive_entity` · `create_entity` · `get_entity` · `list_entities` · `remove_entity_reference` · … |
| `client.files` | `Files` / `AsyncFiles` | 8 | `archive_file` · `create_file_upload` · `disable_public_preview` · `enable_public_preview` · `get_file` · `get_files` · … |
| `client.projects` | `Projects` / `AsyncProjects` | 9 | `create_timeline_interchange` · `export_project` · `get_project` · `get_project_export` · `get_timeline_interchange` · `list_project_exports` · … |
| `client.resources` | `Resources` / `AsyncResources` | 2 | `list_languages` · `list_tts_voices` |
| `client.text` | `Text` / `AsyncText` | 1 | `generate_text` |
| `client.tools` | `Tools` / `AsyncTools` | 16 | `cancel_tool_execution` · `generate_avatar` · `generate_image` · `generate_motion_graphic` · `generate_music` · `generate_sound_effect` · … |
| `client.webhooks` | `Webhooks` / `AsyncWebhooks` | 3 | `create_webhook_endpoint` · `delete_webhook_endpoint` · `list_webhook_endpoints` |
| `client.workflows` | `Workflows` / `AsyncWorkflows` | 7 | `cancel_workflow_run` · `get_workflow_run` · `list_workflow_runs` · `prompt_to_video_clip` · `script_to_video` · `slideshow_to_video` · … |
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `videogen/` source directory, and these two entries:
...
**Installing the distribution does not give you the map.** `pyproject.toml` ships the `videogen/` package only, so `sdk-map.md` and `map/operations/` are absent from the installed package — they live in the SDK's own source tree, at the root of its repository, <https://github.com/sdks-io/vgen-python-sdk>. One clone therefore brings the map and the code it describes together, in lockstep by construction. Clone it to a temporary directory, outside the project repo:

```bash
git clone --depth 1 https://github.com/sdks-io/vgen-python-sdk
```

No branch was recorded for this SDK, so that clone takes the repository's **default** branch. Confirm that is the branch this SDK is released from before you read anything out of it — a map read from the wrong branch describes code you do not have.

Every `Source` path on the map is relative to that SDK root, so `videogen/models/act_on_assistant_action_request.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
Failing that, it is under the project's environment (`.venv/Lib/site-packages/videogen` on Windows, `.venv/lib/python3.*/site-packages/videogen` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
