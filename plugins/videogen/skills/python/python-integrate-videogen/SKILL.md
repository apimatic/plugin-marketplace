...
- **`python-getting-started`** is the entry point for every SDK need. It carries the SDK's identity (distribution name vs import root, version, Python floor, install), what the root package exports and the four modules the surface splits across, environments and the base-URL knob, the auth pattern, the controllers, and a **module table** naming the one file that owns each kind of fact. Load it first, always.
...
**If the user opens with a reported SDK error or unexpected Videogen behaviour** (not new feature work), skip the plan-first flow: load `python-getting-started` and `python-error-handling`, look the failing symbol up in the module the table names, and fix from what you find. Otherwise, for implementation work:

### Step 1 — Plan first (always, for any implementation work)

Your FIRST action is to load `python-getting-started` and work through the user's full request (all features in scope — one plan covers the whole implementation). Then write the plan and its contract sheet to `<project repo root>/videogen-plan.md` — that absolute path, not a location you pick later.
...
- **establish the toolchain before you need it**: which environment manager the repo uses (`uv`, `poetry`, `pip` + venv, `pdm`), whether `videogen` is already a dependency and at what version, and the exact commands that run the project's tests and type checker. Getting this wrong later costs a broken install mid-implementation — and an uninstalled package costs you the lookup layer itself;
...
- credentials/environment verification (per the task's secret-handling rules) — including which host `base_url` will select, since omitting it is silent;
...
- Blocker or major assumption → surface it to the user in plain language, get their answer, and revise `videogen-plan.md` in place.
...
1. Read `videogen-plan.md` once. Treat its contracts as authoritative — do not re-derive or "double-check" them from memory. When a lookup revises a row, update the file so the sheet and the code never disagree.
...
A standalone Videogen question with no code change: look it up in `python-getting-started` (and the module its table names), then give the grounded answer. Never answer from memory, even for "easy" questions.
...
- **Never write a Videogen/SDK fact from memory** — every signature, field name, enum member, and error union in your code must come from the contract sheet or a lookup. And **never write a call from memory "to fix later".**
- **Don't web-search Videogen topics to find an implementation detail** — the installed package's own source is the ground truth, and the module table tells you where to look. Public Videogen docs describe the REST API, not this SDK's generated surface, and the two disagree on names.
