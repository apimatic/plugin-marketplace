...
The **SDK map is bundled with the plugin**, and a single `paypal-platforms-team` agent grounds every fact in it —
cloning the SDK source only when the map can't settle a fact. Structure adopted from the
`maxio-sdk` plugin (an `integrate-paypal` router plus one `paypal-platforms-team` agent).
...
- **integrate-paypal** — orchestrator/router. Routes every PayPal .NET SDK task — planning, contract
  questions, and SDK errors — to the single `paypal-platforms-team` agent, and drives the implement-and-verify
...
- **paypal-platforms-team** — the single SDK agent. Grounds every contract in the bundled map (cloning the SDK
...
/plugin install paypal-platforms-team@apimatic
```
