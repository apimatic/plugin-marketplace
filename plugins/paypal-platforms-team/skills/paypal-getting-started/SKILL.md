...
temp directory** (`<temp>/paypal-platforms-team-src/`), never in the project
...
under `<temp>/paypal-platforms-team-src/`, and reuse that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/paypal-platforms-team-src/$(date +%Y%m%d-%H%M%S)"
...
```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\paypal-platforms-team-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch v1.0.1 https://github.com/asadali214/checkout-sample-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/paypal-platforms-team-src/`, **never** in the project repo, and its path never goes into
