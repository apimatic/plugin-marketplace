...
temp directory** (`<temp>/twilio-platforms-team-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/twilio-platforms-team-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/twilio-platforms-team-src/$(date +%Y%m%d-%H%M%S)"
...
```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\twilio-platforms-team-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/twilio-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/twilio-platforms-team-src/`, **never** in the project repo, and its path never goes into
