...
The **SDK map is bundled with the plugin**, and a single `maxio-platforms-team` agent grounds every fact in it —
cloning the SDK source only when the map can't settle a fact. Sourced from
[apimatic/v4-plugins](https://github.com/apimatic/v4-plugins) `plugins/maxio-platforms-team`, extended with
**subagent orchestration** (an `integrate-maxio` router plus one `maxio-platforms-team` agent).
...
- **integrate-maxio** — orchestrator/router. Routes every Maxio .NET SDK task — planning, contract
  questions, and SDK errors — to the single `maxio-platforms-team` agent, and drives the implement-and-verify
...
- **maxio-platforms-team** — the single SDK agent. Grounds every contract in the bundled map (cloning the SDK
...
/plugin install maxio-platforms-team@apimatic
```
