...
description: Maxio Advanced Billing (formerly Chargify) .NET SDK identity and lookup layer for the maxio-platforms-team helper agent — package id, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Maxio Advanced Billing .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `maxio-platforms-team` helper
...
temp directory** (`<temp>/maxio-platforms-team-src/`), never in the project
...
the map was generated from) — into a fresh timestamped folder under `<temp>/maxio-platforms-team-src/`, and reuse that
folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/maxio-platforms-team-src/$(date +%Y%m%d-%H%M%S)"
...
```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\maxio-platforms-team-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch v1.0.2 https://github.com/asadali214/advanced-billing-sample-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/maxio-platforms-team-src/`, **never** in the project repo, and its path never goes into
