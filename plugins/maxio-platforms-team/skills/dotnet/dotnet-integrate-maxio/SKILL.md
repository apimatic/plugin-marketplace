...
**If the user opens with a reported SDK error or unexpected Maxio Advanced Billing behaviour** (not new feature work), go straight to *Step 4 — Fixing SDK errors* below — do not run the plan-first flow for a bug report. Otherwise, for implementation work:

### Step 1 — Plan first: write `maxio-advanced-billing-plan.md` (always, for any implementation work)

Your FIRST deliverable is `maxio-advanced-billing-plan.md` at `<project repo root>/maxio-advanced-billing-plan.md` — that exact path, always; never pick another location. It carries the plan and the contract sheet in the format below, and it is the only project-repo file you write before the HARD GATE further down.
...
**No project-file creation or edits until `maxio-advanced-billing-plan.md` exists at that path with every section below filled in.** Starting to code before the sheet exists defeats the plan-first design: the sheet is what keeps SDK facts out of your memory and in the map. The gate bars coding before the sheet exists; it does not bar the read-only prerequisite work above.

#### The `maxio-advanced-billing-plan.md` format (keep it tight — tables, not prose)
...
2. **CONTRACT SHEET** — open the section with two literal warning lines: that signatures are generated code, verbatim, every parameter name the literal C# identifier (the cancellation-token parameter really is named `ct`, so named arguments write `ct:`); and that every SDK type is written fully-qualified with the namespace its source path implies, taken from the path the map gives for THAT type, never from where a neighbouring type sits. Then one table row per operation: controller property · method signature · request model + its fields (`Name (wire_name): type, required?`) · response envelope + the inner fields the integration reads · error case A/B + accessors + payload type · pagination · **source** (§7). Below it: the enum value tables actually needed, and the client construction/auth/server-node facts. ⚠ A request model may mark nothing required, and then `required?` selects nothing for you — carry the optional fields the endpoint's own prose ties to whether the call is accepted, and say which doc-named fields you left out. ⚠⚠ **Every optional field on a create/update body carries a `purpose` note, or it does not go on the sheet** — a bare `Name (wire_name): type` says what a field *is* and nothing about when to set it, and an optional field set unnecessarily overrides a provider-side default. Two notes are mandatory where they apply, because neither is inferable from the type: **`omit → provider default`** and **`import/migration only`**. If the source says neither, label the row `UNVERIFIED` and omit the field in code.
   Then, **below the per-operation rows, a `CROSS-OPERATION INVARIANTS` table** — the rules no single operation row can state, because they relate two operations to each other. One row per value the task lets a caller supply where a *different* operation defines the legal set for it (identifiers, handles, codes, references):

   | invariant | operations | enforced where |
   | --- | --- | --- |
   | a value accepted by X must be one Y returned | `{WriteOperation}` ← `{ListOperation}` | implementation |

   ⚠⚠ **`YOUR CALL — not in the map` is not a legal entry in this table.** The map describes operations one at a time, so it can never state a rule spanning two of them: its silence here is expected, and is not a reason to leave the rule unwritten. Enforcement belongs to the application and follows its existing validation conventions; deriving the invariant is still your job. If the task implies no such pairing, write `none` explicitly.
...
5. **PRODUCTION READINESS** — the fixed fourteen-row table below, every row carrying a *decision*. Naming a skill in REQUIRED READING does not address a concern; it defers it. `N/A` is a legitimate answer where it is genuinely true, but it must carry its reason.
...
| 9 | **Duplicate prevention under concurrency** | For each write a caller can trigger twice: the **name of the store and the column** carrying the claim, and that its **unique constraint** rejects the second row and the code **catches that rejection**. ⚠ `AsyncLocal`, `SemaphoreSlim`, an in-memory dictionary, a single-host assumption, or an existence check before the write are **not** acceptable entries, and neither is documenting one as a known limitation. If the codebase can carry no such claim, this is a Blocker in §6, not a row. |
| 10 | **Partial results** | For each paged read that can hit a cap: the **field or return type** by which the caller learns the result was truncated. ⚠ A log line is **not** an acceptable entry. |
| 11 | **Startup validation vs the existing test host** | If a startup credential check is added: **which test project boots the host**, what it was given (placeholder configuration or a stub registration), and that **it was run and is green**. ⚠ "Unit tests pass" is **not** an acceptable entry when the host-booting project was never executed. |
| 12 | **Ordering and no-op side effects** | That the local record is written **before** the provider call and completed after it; and for each idempotent transition, **which outbound effects are gated** on whether state actually changed. ⚠ Persisting the provider's response as the first local write is **not** acceptable, and neither is an unconditional notification after a transition. |
...
Every one of them ends in a **`where in the code`** column. ⚠⚠ **Write `TBD` in that cell while planning — every row, no exceptions.** Filling it at planning time is guessing, and the guesses are wrong: one run named a service class in all six tables that it then never created, and every cell in the file pointed at a type that does not exist. Another returned at the end and replaced its rows with a single note saying they all lived in one file, which discharges nothing.

So: `TBD` while planning; then, once the code compiles and before you report the work finished, **open the plan and replace each `TBD` individually** with the member that does what that row promised — read from the code you actually wrote, one row at a time. ⚠⚠ **A row still reading `TBD`, a row naming something that is not in the code, and a blanket note covering several rows at once are all *not addressed*.** If filling a row shows the code does not do what the row says, the code is what is wrong; fix it and then fill the cell.
...
⚠⚠ Three entries are **not legal** in the third column. **A value that means the provider has not finished is not success** — every such value belongs in its own outcome, and grouping any of them with the values that mean the work is done is this row's defect written out in full. Read the enum or the field's documented values and sort them into three groups yourself: done, not-yet, and failed; if a value's meaning is not settled by the source, it is not-yet. Defaulting an absent status to a successful one — coalescing a null status to whichever member means success — turns an outcome you could not read into the one outcome you wanted, and an unreadable status is not-yet as well. Passing the status into a method that does not branch on it is not reading it. And if the operation returns an identifier, that is not an outcome either: the identifier says the provider received the request, not that it did the thing.
...
- Blocker or major assumption → surface it to the user in plain language, get their answer, then revise `maxio-advanced-billing-plan.md` in place with targeted edits — edit the changed rows, append the new section. Rewriting the whole file to change a few rows is a defect.
...
A compile or runtime error involving an SDK type or member (`CS1061`, `CS0117`, `CS0234`, `CS0104`, `CS1503`, `CS7036`, … on `MaxioAdvancedBilling.*`, or a provider error). Resolve it map-first — never patch it by guessing. Load `dotnet-getting-started` first if you have not already — the map is where every fix below comes from.
...
If a fix corrects a row in `maxio-advanced-billing-plan.md`, correct the row in the file too — the sheet stays the record of what was verified.

### Step 5 — Pure questions

A standalone Maxio Advanced Billing question with no code change: load `dotnet-getting-started`, look it up in the map (open the one source file the map names only on a real gap), and answer with the map page (or source file) it came from. No plan file — just the grounded answers; when several questions arrive batched, answer them all in one pass. Never answer from memory, even for "easy" questions.

## Anti-patterns — never do these

- **Never write a Maxio Advanced Billing/SDK fact from memory** — every signature, field name, enum value, and error type in your code must come from the contract sheet or a map lookup. And **never write a call from memory "to fix later".**
...
- **Do not web-search Maxio Advanced Billing for an implementation detail** — the map and the pinned source are the ground truth for THIS SDK version, and `MaxioAdvancedBilling` is what the generated source declares, not what public docs describe.
