...
- **`maxio-platforms-team`** is the single agent for every SDK need. It grounds in a bundled SDK map
...
feature work), spawn **`maxio-platforms-team`** directly with the error output and the files involved,
...
Your FIRST action is to spawn **`maxio-platforms-team`** once, with the user's full request (all
...
lands in this codebase) is not the `maxio-platforms-team` agent's job: kick it off in the SAME message
...
- Blocker or major assumption → surface it to the user in plain language, get their answer,
  send the clarification to the EXISTING `maxio-platforms-team` agent (re-spawn only if it is gone). It
...
ask the warm `maxio-platforms-team` agent, never guess.
...
2. Implement sequentially, following the repo's own conventions and layering. You loaded the
   ...
   contract sheet or the warm `maxio-platforms-team` agent — never re-derive one from a companion.
...
4. **Any compile or runtime error involving an SDK type or member** (`CS1061`, `CS0117`,
   ...
   `maxio-platforms-team` agent (a follow-up message, NOT a new spawn), and wait. It fixes the code in
...
A standalone Maxio question with no code change: ask the warm `maxio-platforms-team` agent (narrow-
question mode) and relay its grounded answer. Never answer from memory, even for "easy"
questions.

## Wait for your agent

**Never create or edit a project file while `maxio-platforms-team` is running** — it edits files in
...
- **Get SDK knowledge from the `maxio-platforms-team` agent, not yourself.** Don't clone or decompile the
...
- **Don't load `maxio-getting-started` or the SDK map pages** — the map is the agent's, and
  ...
  error accessors, and enum values come from the contract sheet (or you ask the warm `maxio-platforms-team`
...
- **Don't re-derive or double-check a sheet row from memory.** If you are unsure what a row
  said, batch your questions into ONE message to the warm `maxio-platforms-team` agent — it holds the map
  warm and answers in seconds.
- **Never spawn a second `maxio-platforms-team` agent.** One spawn per session; everything after is a
