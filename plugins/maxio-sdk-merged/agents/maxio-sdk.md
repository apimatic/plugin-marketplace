...
2. **CONTRACT SHEET** — open the section with these two literal warning lines:
   ...
   accessors + payload type · pagination. Below it: the client construction/auth/server-node facts,
   and **the full value table for every enum-typed field of every request model the sheet names** —
   all of them, not the ones you judge the implementer will need. You are the only channel: a value
   you leave out is unrecoverable downstream, because the implementer is barred from the map. Before
   you finish, re-read your sheet against the map rows you opened and confirm every enum-typed field
   of every named request model has its values listed. Do not write a rationale for omitting a
