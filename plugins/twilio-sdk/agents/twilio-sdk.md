...
5. **PRODUCTION READINESS** — a fixed eight-row table, every row carrying a *decision*. Naming a
   ...
   | 8 | **Environment selection** | Which base URL each deployment talks to — and note that there is only one. `ServerEnvironment` has exactly **one** member, `Production`, and every call resolves through `Server.Default.Production.BaseUrl` (`https://api.twilio.com`). **This SDK has no sandbox.** A dev run, an integration test and a production deployment all reach live Twilio unless you point `BaseUrl` somewhere else yourself: real messages sent, real charges. Say what each environment does about that. |
...
7. Every sheet row cites its map page (e.g. `operations/Api20100401Message.md`, `records-1-Ac-Ca.md`)
