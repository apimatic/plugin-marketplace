---
name: integrate-twilio
description: MANDATORY FIRST STEP for Twilio .NET SDK work in a C#/.NET project — load this BEFORE opening the SDK map or touching any project file; .NET/C# SDK ONLY, never load it for any other language. Applies when asked to integrate Twilio in C# — any feature that creates, reads, updates, searches or deletes Twilio resources through this SDK — or when a Twilio .NET SDK call errors or behaves unexpectedly. It carries four binding rules stated NOWHERE else — (1) for implementation work, the plan file twilio-plan.md is written at <project repo root>/twilio-plan.md BEFORE any project file is created or edited; (2) twilio-getting-started and the SDK map it carries are loaded and used for every contract fact — never memory; (3) every dotnet-* skill the contract sheet's REQUIRED READING names is loaded before implementation starts; (4) every sheet row carries one of three labels — a map page, UNVERIFIED, or YOUR CALL — not in the map. Skip it and SDK facts get written from stale memory.
---

# Twilio .NET SDK — Workflow (map + skills)

You are the Twilio .NET SDK specialist for this task — one agent for every SDK need: planning,
...
**bundled SDK map** inside the `twilio-getting-started` skill (`sdk-map.md` +
...
`twilio-getting-started`). You never guess, and you never open the SDK's `api-reference.md`. You
...
C#/.NET — do nothing; this skill does not apply.

## Grounding rules

- **Map first; source only on a real gap.** Follow `twilio-getting-started`'s *SDK map* and *SDK
  ...
  appears in `twilio-plan.md` — the plan must stay portable.
...
- **The sheet records Twilio's call surface; the application's design is decided at implementation
...
**If the user opens with a reported SDK error or unexpected Twilio behaviour** (not new feature
...
### Step 1 — Plan first: write `twilio-plan.md` (always, for any implementation work)

Your FIRST deliverable is `twilio-plan.md` at `<project repo root>/twilio-plan.md` — that exact
...
1. Load `twilio-getting-started`; open `sdk-map.md` (the index).
...
**No project-file creation or edits until `twilio-plan.md` exists at that path with every section
...
#### The `twilio-plan.md` format (keep it tight — tables, not prose)
...
4. **REQUIRED READING** — close the sheet with the de-duplicated list of every `dotnet-*` skill
   ...
   (`twilio-sdk:dotnet-error-handling`): three plugins in this marketplace ship these same seven
...
5. **PRODUCTION READINESS** — a fixed eight-row table, every row carrying a *decision*. Naming a
   ...
   | 2 | **Secret sourcing & rotation** | Where the secret comes from, and that `AddTwilioSdkClient` builds the options object **once at registration** and captures it in the singleton — so a rotated secret does not take effect until the process restarts. If rotation without a restart is required, say how. |
   ...
   | 7 | **Sensitive data** | Whether the scope carries data you would not want in a log — the map's model pages list every request field in scope (card or account numbers, personal data, message content). If it does: `LogRequestBody` stays off **and** `LoggerFactory` is assigned explicitly, so `TWILIOSDKCLIENT_LOG=trace` cannot switch it on from outside the code. Form bodies are masked only by deny-list, so a key you have not listed prints in the clear. |
...
- Blocker or major assumption → surface it to the user in plain language, get their answer, then
  revise `twilio-plan.md` in place with targeted **Edit** operations — edit the changed rows,
...
1. Read `twilio-plan.md` once. Treat its contracts as authoritative — do not re-derive or
...
`CS0104`, `CS1503`, `CS7036`, … on `TwilioSdk.*`, or a provider error). Resolve it map-first —
never patch it by guessing. Load `twilio-getting-started` first if you have not
...
2. **Source only on a real gap.** If the map row matches the code, or ambiguity remains, clone
   the SDK per `twilio-getting-started`'s *SDK source* section (reuse this session's clone if you
...
If a fix corrects a row in `twilio-plan.md`, correct the row in the file too — the sheet stays the
record of what was verified.

### Step 5 — Pure questions

A standalone Twilio question with no code change: load `twilio-getting-started`, look it up in
...
- **Don't grep, glob or `find` the SDK tree, and don't open its `api-reference.md`** — the map is
  the locator; `twilio-getting-started` says why.
- **Don't web-search Twilio for an implementation detail** — the map and the pinned source are the
