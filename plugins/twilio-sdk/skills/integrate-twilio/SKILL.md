...
- **`twilio-sdk`** is the single agent for every SDK need. It grounds in a bundled SDK map
...
feature work), spawn **`twilio-sdk`** directly with the error output and the files involved,
...
Your FIRST action is to spawn **`twilio-sdk`** once, with the user's full request (all
...
- Blocker or major assumption → surface it to the user in plain language, get their answer,
  send the clarification to the EXISTING `twilio-sdk` agent (re-spawn only if it is gone). It
...
ask the warm `twilio-sdk` agent, never guess.
...
A standalone Twilio question with no code change: ask the warm `twilio-sdk` agent (narrow-
question mode) and relay its grounded answer. Never answer from memory, even for "easy"
questions.

## Wait for your agent

**Never create or edit a project file while `twilio-sdk` is running** — it edits files in
...
- **Get SDK knowledge from the `twilio-sdk` agent, not yourself.** Don't clone or decompile the
...
- **Don't load `twilio-getting-started` or the SDK map pages** — the map is the agent's, and
  ...
  error accessors, and enum values come from the contract sheet (or you ask the warm `twilio-sdk`
...
- **Don't re-derive or double-check a sheet row from memory.** If you are unsure what a row
  said, batch your questions into ONE message to the warm `twilio-sdk` agent — it holds the map
  warm and answers in seconds.
- **Never spawn a second `twilio-sdk` agent.** One spawn per session; everything after is a
