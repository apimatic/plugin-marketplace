...
<!-- crawler:auth -->
| Auth | Credentials properties on `TwilioClientOptions`: `AccountSidAuthToken: BasicAuthCredentials?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using Twilio.Models;` alone does **not** make enums, union
...
dotnet add reference twilio-csharp-sdk/Twilio.csproj
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new TwilioClient(...)`, build its options, or DI-register via
   `AddTwilioClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `TwilioClientOptions` — the map's *Servers & auth* section
